...
from models import Client, ClientResponse, ClientCreateRequest, ClientUpdateRequest, ClientStatus
...
class ClientService:
    ...
    async def _check_client_uniqueness(self, pan_number: str, client_name: str, user_google_id: str, exclude_client_id: str = None):
    ...
    async def create_client(self, client_data: ClientCreateRequest, user_google_id: str) -> ClientResponse:
        """Create a new client in the database."""
        await self._ensure_db_connection()
        try:
            # Check client uniqueness (user_google_id + pan_number + client_name)
            await self._check_client_uniqueness(client_data.pan_number, client_data.name, user_google_id)
            
            # Create client document
            client_dict = client_data.model_dump()
            client_dict["user_google_id"] = user_google_id
    ...
    async def get_client_by_id(self, client_id: str, user_google_id: str) -> Optional[ClientResponse]:
    ...
    async def get_clients_by_user(self, user_google_id: str, skip: int = 0, limit: int = 100) -> List[ClientResponse]:
        """Get all clients for a specific user."""
        await self._ensure_db_connection()
        try:
            cursor = self.clients_collection.find({"user_google_id": user_google_id}).skip(skip).limit(limit)
            clients = []
    ...
    async def update_client(self, client_id: str, client_data: ClientUpdateRequest, user_google_id: str) -> Optional[ClientResponse]:
        ...
        try:
            ...
            if client_data.pan_number or client_data.name:
                ...
                current_client = await self.get_client_by_id(client_id, user_google_id)
                ...
                await self._check_client_uniqueness(pan_to_check, name_to_check, user_google_id, client_id)
            ...
            if not update_data:
                # No fields to update
                return await self.get_client_by_id(client_id, user_google_id)

            update_data["updated_at"] = datetime.utcnow()

            result = await self.clients_collection.update_one(
                {"_id": ObjectId(client_id), "user_google_id": user_google_id},
                {"$set": update_data}
            )
    ...
    async def delete_client(self, client_id: str, user_google_id: str) -> bool:
    ...
    async def search_clients(self, user_google_id: str, search_query: str, skip: int = 0, limit: int = 100) -> List[ClientResponse]:
    ...
    async def get_client_count(self, user_google_id: str) -> int:
        """Get total number of clients for a user."""
        await self._ensure_db_connection()
        try:
            count = await self.clients_collection.count_documents({"user_google_id": user_google_id})
            return count
        except Exception as e:
            logger.error(f"Error getting client count: {e}")
            return 0

    async def get_clients_by_status(self, user_google_id: str, status: str, skip: int = 0, limit: int = 100) -> List[ClientResponse]:
    ...
    async def get_client_statistics(self, user_google_id: str) -> dict:
        """Get client statistics for dashboard metrics."""
        await self._ensure_db_connection()
        try:
            # Get all clients for the user
            total_clients = await self.clients_collection.count_documents({"user_google_id": user_google_id})
